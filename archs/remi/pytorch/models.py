...
from archs.remi.modules import RelPartialLearnableDecoderLayer,AdaptiveEmbedding, \
                                RelLearnableDecoderLayer, DecoderLayer, ProjectedAdaptiveLogSoftmax, PositionalEmbedding
...
class Transformer(nn.Module):
    ...
    #  ['C', 'G', 'Am', 'Em', 'F', 'C', 'Dm', 'G']
    # ['G', 'D', 'Em', 'Bm', 'C', 'G', 'Am', 'D']
    def generate(self, primer_tokens, vocab, n_bar, num_syll, topk= 2, temperature = 0.5, device='cuda:0', 
                chord_progress =[], **kwargs):
        mems = self.init_mems()
        self.eval()
        initial_flag = True
        predicted_bar = 0
        ...
        if len(chord_progress) != 0:
            chord_progress = itercycle(chord_progress)
        ...
        if len(primer_tokens) == 0:
            ...
            last_bar_idx = np.where(np.array(primer_tokens)==vocab.bar_idx)
            # import pdb; pdb.set_trace()
            try:
                cur_predicted_bar_tokens = full_tokens[last_bar_idx[0][-1]:].copy()
            except: 
        ...
        cur_pos_inbar = []
        for pos in cur_predicted_bar_tokens:
            if pos in vocab.position_idx_range[0]:
                cur_pos_inbar.append(pos)  
        full_tokens = []
        pred_tokens = []
        total_bar_seq = []
        # self.reset_length(10,0,10)
        if num_syll == None: num_syll = float('-Inf')
        while (predicted_bar < n_bar) or (predicted_syll < num_syll):
            #continue predict while not enough both syllables and bar
            
            topk = int(np.random.choice([1,2], p=[0.6, 0.4]))
            if initial_flag:
                with torch.no_grad():   
                ...
                # if prev_token in chords, replace with chord progress
                if prev_token in vocab.chord_idx_range[0]:
                    
                    prev_token = self._replace_if_chord(prev_token, vocab, chord_progress)
                ...
                with torch.no_grad():
                    self.reset_length(1,0,40) ### Not sure it work
            ...
            # check whether token predicted is position: 
            if prev_token in vocab.position_idx_range[0]:
                # Do nothing
                pass
                
            # check whether token predicted is Note
            elif prev_token in vocab.pitch_idx_range:
                # Do nothing    
                pass

            # check whether token predicted is Bar token
            elif prev_token == vocab.bar_idx:
                try: 
                    bar_seq = item_processor.tokens_to_barsequence(np.array(cur_predicted_bar_tokens), vocab)
                    timestep = [x.start for x in bar_seq[::][0] if isinstance(x,Note)]
                except:
                    continue
                
                full_tokens = full_tokens + cur_predicted_bar_tokens[:-1]
                count = len(list(set(timestep)))
                predicted_syll += count
                predicted_bar += 1
                print('{}/{}'.format(predicted_bar, n_bar))
                print(vocab.textify(cur_predicted_bar_tokens))
                unique_pos = np.unique(cur_pos_inbar)
                cur_predicted_bar_tokens = [0]

        full_tokens.append(0)
